...
var bindParser = function(p, f) {
    return function BIND_PARSER(state, cok, cerr, eok, eerr) {
        var pcok = function(v, state) { return cont(f(v, state), [state, cok, cerr, cok, cerr]); },
            peok = function(v, state) { return cont(f(v, state), [state, cok, cerr, eok, eerr]); };
...
var exec = function(p, state, cok, cerr, eok, eerr) {
    return trampoline(p(state, cok, cerr, eok, eerr))();
};
...
var runManyState = function(p, state) {
    var manyP = _optionalValueParser(
        bindParser(p, function(v, state) {
            return parse.always(stream.stream(v, bind(runState, manyP, state)));
        }));
