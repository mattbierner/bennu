...
var manyParser = (function(){
    ...
    return function(p) {
        ...
        var many;
        return (many = eitherParser(
            bindParser(safeP, function(x) { return _concatParser(many, x); }),
            _end));
    };
}());
...
var optionalParser = (function(){
    ...
    return function(p) {
        return eitherParser(attemptParser(bindParser(p, wrap)), _end);
    };
}());
...
var betweenTimesParser = (function(){
    var between = function(min, max, p) {
        var minParser = timesParser(min, p);
        return (max === min ? minParser: bindParser(minParser, function (x) {
            return bindParser(optionalParser(between(1, max - min, p)), function(xs) {
    ...
    return function(min, max, p) {
        return (max < min ? neverParser() : between(min, max, p));
    };
}());
 ...
 */
var anyCharParser = constant(tokenParser(test.bind(/^.$/)));

/**
 * Parser that consumes any letter character.
 * 
 */
var letterParser = constant(tokenParser(test.bind(/^[a-z]$/i)));

/**
 * Parser that consumes any space character.
 * 
 */
var spaceParser = constant(tokenParser(test.bind(/^\s$/i)));


/**
 * Parser that consumes any number character.
 */
var digitParser = constant(tokenParser(test.bind(/^[0-9]$/i)));
...
/**
 * Run a given parsers against an input string.
 * 
 * @param p Parser to run.
