...
define(["nu/stream"], function(stream) {
    ...
    var MultipleError = function(pos, errors) {
    ...
    Object.defineProperties(MultipleError.prototype, ({
        "errorMessage": ({
            "get": function() {
    ...
    ;
    var eitherParser = _eitherParser(function(pos, pErr, qErr) {
        return new MultipleError(pos, [pErr, qErr]);
    }
    );
    var choiceParser = function() {
        var eitherParser = _eitherParser(function(pos, pErr, qErr) {
            return new MultipleError(pos, [pErr].concat(qErr.errors));
        }
        );
        ...
        var end = function(state, m, cok, cerr, eok, eerr) {
            return eerr(new MultipleError(state.pos, []), state, m);
        }
        ;
        return function() {
            return ((arguments.length === 0) ? neverParser() : reduceRight.call(arguments, reducer, end));
        }
        ;
    }
    ();
    ...
    var appendParser = curry(_joinParser, stream.append);
    ...
    var betweenTimesParser = function() {
        ...
        return function(min, max, p) {
            return ((max < min) ? neverParser : appendParser(timesParser(min, p), maxParser((max - min), p)));
        }
        ;
    }
    ();
