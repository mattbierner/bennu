...
////////////////////////////////////////
var cont = function(/*...*/) {
    var f = bind.apply(undefined, arguments);
    f._next = true;
    return f;
};
 ...
 * 
 *    var bs = parse.either(parse.char('b'), bs) -> parse.either(parse.char('b'), undefined)
 * 
 * Using RecParser, we fix this.
 * 
 *     var bs = RecParser(function(bs) {
 *         return parse.either(parse.char('b'), bs);
 *     });
 * 
...
/**
 * Parser that consumes character based on a predicate.
 * 
 * 
 * 
