...
define(["nu/stream"], function(stream) {
    ...
    (Memoer.lookup = function(cell, id, state) {
        for(var m = cell;
         m;(m = m.delegate))if (((m.id === id) && m.state.eq(state)))return m.val;
    ...
    var eof = function(end) {
        return bind(getInput, function(input) {
            return (stream.isEmpty(input) ? end : bind(getPosition, function(pos) {
    ...
    var sequence = function(reducer) {
        return function() {
            return reduceRight(arguments, reducer);
        }
        ;
    }
    ...
    var enumeration = function(reducer) {
        return function() {
