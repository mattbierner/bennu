 ...
 * 
 */
var Memoer = function(val, state, delegate) {
    this.val = val;
    this.state = state;
 ...
 */
Memoer.lookup = function(cell, state) {
 ...
 * some cell 'm'.
 */
Memoer.update = function(m, val, state) {
    return new Memoer(val, state, m);
};
...
var memoParser = (function(){
    ...
    return function(p) {
        return function(state, m, cok, cerr, eok, eerr) {
            var entry = Memoer.lookup(m, state);
            if (!entry) {
                entry = trampoline(p(state, m, pcok, pcerr, peok, peerr));
                m = Memoer.update(m, entry, state);
            }
