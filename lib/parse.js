...
var mergeErrors = function(e1, e2) {
    return new ParseError(e1.pos,
        e1.message ? [e1.message, e2.message] : [e2.message]);
};
...
/**
 * Parse p then q. Returns q's value.
 */
...
var token = function(consume) {
    return function(state, cok, cerr, eok, eerr) {
        ...
        if (tok === undefined) {
            ...
            if (consume(tok)) {
                return cok(tok, InputState(rest(input), Position.increment(pos, tok)));
            } else {
                return eerr(new UnexpectError(pos, "token:'" + tok + "'"));
            }
        }
    };
};
...
/**
 * Parser that consumes and number character.
 */
...
var string = function(s) {
    return Array.prototype.reduce.bind(s, function(p, c) {
        return next(p, always(c));
    }, [])
};
...
var Parser = function(body) {
    return function(state, cok, cerr, eok, eerr) {
        return body(state, cok, cerr, eok, eerr);
    };
};
...
var runParser = (function(){
    ...
    return function(p, state) {
        return p(state, cok, cerr, eok, eerr);
    };
}());
