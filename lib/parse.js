...
define(["nu/stream", "seshat"], function(stream, seshat) {
    ...
    (Memoer.pushWindow = function(m, lower, upper) {
        return new Memoer(m.memoer, m.frames.concat([[lower, upper]]));
    }
    );
    (Memoer.popWindow = function(m) {
        return new Memoer(m.memoer, m.frames.slice((m.frames.length - 1)));
    }
    );
    ...
    (Memoer.update = function(m, pos, id, val) {
        var updated = seshat.update(m.memoer, pos, id, val);
        return new Memoer((m.frames[0] ? seshat.prune(updated, m.frames[0][0], m.frames[0][1]) : updated), m.frames);
    }
    );
    ...
    var attempt = function(p) {
        return function ATTEMPT_PARSER(state, m, cok, cerr, eok, eerr) {
            return cont(p, [state, Memoer.pushWindow(m, state.position), cok, eerr, eok, eerr]);
        }
        ;
    }
    ;
    ...
    var memo = function(p) {
        return function(id) {
            return function(state, m, cok, cerr, eok, eerr) {
                ...
                , function(x, pstate, pm) {
                    return eerr(x, pstate, Memoer.update(m, pos, id, function(_, m, cok, cerr, eok, eerr) {
