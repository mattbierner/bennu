...
var InputState = function(input, pos) {
    return Object.create(Object.prototype, {
        ...
        'first': {
            'value': bind(first, input)
        },
        'rest': {
            'value': bind(rest, input)
        },
        'pos': {
            'value': pos,
            'writable': false
    ...
    return function(p) {
        ...
        var many = eitherParser(
            bindParser(safeP, function(x) {
                return bindParser(many, function(xs) {
    ...
    if (n <= 0) {
        ...
        return function(state, cok, cerr, eok, eerr) {
            var pcok = function(item, state) {
                ...
                return cont(timesParser(n - 1, p), state, qcok, cerr, qcok, eerr);
            };
...
var runParser = (function(){
    ...
    return function(p, state) {
        return trampoline(cont(p, state, Ok, Err, Ok, Err));
    };
}());
