...
define(["require", "exports", "nu/stream", "seshat"], (function(require, exports, stream, seshat) {
    "use strict";
    var ParserError, ParseError, MultipleError, UnknownError, UnexpectError, ExpectError, ParserState, Position, rec, Parser, RecParser, always, never, bind, eof, extract, getParserState, setParserState, modifyParserState, getState, setState, modifyState, getInput, setInput, getPosition, setPosition, fail, attempt, lookahead, next, sequencea, sequence, either, choicea, choice, optional, expected, eager, binds, cons, append, enumerationa, enumeration, many, many1, token, anyToken, memo, Memoer, exec, perform, parse, parseState, finish, provide, provideString, runState, runStream, run, runManyState, runManyStream, runMany, testState, testStream, test;
    var stream = stream,
        NIL = stream["end"],
        first = stream["first"],
        ...
        seshat = seshat; {
            ...
            (perform = (function(p, state, ok, err) {
                return _perform(p, state, (function(x, state, m) {
                    return (function() {
                        return ok(x, state, m);
                    });
                }), (function(x, state, m) {
                    return (function() {
                        return err(x, state, m);
                    });
                }));
            }));
            var Request = (function(done, state, k) {
                (this.done = done);
                (this.state = state);
                (this.k = k);
            });
            (provide = (function(r, c) {
                return (r.done ? r : new(Request)(false, r.state.setInput(stream.append(r.state.input, c)), r.k));
            }));
            (provideString = (function(r, s) {
                return provide(r, stream.from(s));
            }));
            (finish = (function(r) {
                return (!r.done ? finish(r.k(r.state)) : trampoline(r.k(r.state)));
            }));
            (parseState = (function(p, state) {
                return new(Request)(false, state, (function(s) {
                    return _perform(p, state, (function(x, s) {
                        return new(Request)(true, s, (function() {
                            return x;
                        }));
                    }), (function(x, s) {
                        return new(Request)(true, s, (function() {
            ...
            (parse = (function(p, ud) {
                return parseState(p, new(ParserState)(stream.end, Position.initial, ud));
            }));
            (runState = (function(p, state) {
                return finish(provide(parseState(p, state), state));
            }));
