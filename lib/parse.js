...
// Iterative  Parsers
////////////////////////////////////////
var _consParser = function(p1, p2) {
    return bindParser(p1, function(x) {
        return bindParser(p2, function(xs) {
...
var many1Parser = function(p) {
    return _consParser(p, manyParser(p));
};
...
var timesParser = function(n, p) {
    return (n <= 0 ?
        _end : 
        _consParser(p, timesParser(n - 1, p)));
};
    ...
    var maxParser = function(max, p) {
        return (max <= 0 ?
            ...
            _optionalValueParser(
                _consParser(p, maxParser(max - 1, p))));
    };
...
var sepBy1 = function(sep, p) {
    return _consParser(p, manyParser(nextParser(sep, p)));
};
