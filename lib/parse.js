...
define(["nu/stream"], function(stream) {
    ...
    ;
    var Memoer = ({});
    (Memoer.create = function(id, state, val, delegate, window) {
        return ({
            "cell": ({
                "id": id,
                "state": state,
    ...
    (Memoer.lookup = function(m, id, state) {
        for(var cell = m.cell;
         cell;(cell = cell.delegate))if (((cell.id === id) && cell.state.eq(state)))return cell.val;
        
        
        return null;
    }
    );
    (Memoer.prune = function(m) {
        var cell = m.cell;
        var i = 0;
        while((cell && (! m.window[0] || (m.window[0].compare(cell.state.position) < 0)))){
            (cell = cell.delegate);
            (i = (i + 1));
        }
    ...
    (Memoer.update = function(m, id, state, val) {
        ...
        ;
        console.log(("pruned:" + i));
        return Memoer.create(id, state, val, cell, m.window);
    }
    );
    (Memoer.setWindow = function(m, window) {
        return (! m.cell ? ({
            "window": window
        }) : Memoer.create(m.cell.id, m.cell.state, m.cell.val, m.cell.delegate, window));
    }
    );
    ...
    (ParserState.prototype.eq = function(__a) {
        var pos = __a["position"];
        return (this.position.compare(pos) === 0);
    }
    );
    ...
    ;
    var getParserState = extract(identity);
    ...
    ;
    var getState = extract(function(s) {
        return s.userState;
    }
    );
    ...
    ;
    var getPosition = extract(function(s) {
        return s.position;
    }
    );
    ...
    ;
    var getInput = extract(function(s) {
        return s.input;
    }
    );
    ...
    var attempt = function(p) {
        return function ATTEMPT_PARSER(state, m, cok, cerr, eok, eerr) {
            return cont(p, [state, Memoer.setWindow(m, [state.position]), cok, eerr, eok, eerr]);
        }
        ;
    }
    ;
    ...
    var choice = function(either) {
        return function(reducer) {
            return function(end) {
                return function() {
                    if (! arguments.length)throw new ParserError("choice called with no arguments");
