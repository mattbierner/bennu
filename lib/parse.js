...
var bindParser = function(p, f) {
    return function BIND_PARSER(state, m, cok, cerr, eok, eerr) {
        var pcok = function(x, state, m) { return cont(f(x, state, m), [state, m, cok, cerr, cok, cerr]); },
            peok = function(x, state, m) { return cont(f(x, state, m), [state, m, cok, cerr, eok, eerr]); };
