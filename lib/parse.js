...
Stream.cons = function(item, sequence) {
    return {
        'first': item,
        'rest': sequence
    };
...
Stream.toArray = function(s) {
    var r = [];
    for (var elm = s; elm !== null; elm = elm.rest) {
        r.push(elm.first);
    }
    return r;
};
...
// StringStream
////////////////////////////////////////
var StringStream = function(s, i, len) {
    this.s = s;
    this.i = i;
    this.length = len;
    this.first = this.s[this.i];
};

StringStream.prototype.rest = function() {
    return (this.i + 1 === this.length ?
        null :
        new StringStream(this.s, this.i + 1, this.length));
};

var asStringStream = function(s) {
    return (s === undefined || s.length === 0 ?
        null :
        new StringStream(s, 0, s.length));
};
...
InputState.prototype.next = function(tok) {
    return (this._next || (this._next = new InputState(this.input.rest(), Position.increment(this.pos, tok))));
};
...
var run = function(p, input) {
    return runStream(p, asStringStream(input));
};
...
var test = function(p, input) {
    return testStringStream(p, asStringStream(input));
};
...
var bindaParser = function(p, f) {
    return bindParser(p, apply.bind(f, undefined));
};
...
var toArrayParser = function(p) {
    return bindParser(p, function(v) {
        return alwaysParser(Stream.toArray(v));
    });
};
 ...
 * @return List of 'p' results. May be empty if zero 'p' was consumed.
 */
var manyParser = function(p) {
    return toArrayParser(manyStreamParser(p));
};
...
var many1Parser = function(p) {
    return toArrayParser(bindParser(p,
        bind(_concatParser, manyStreamParser(p))));
};
 ...
 * @return List of 'p' results.
 */
var timesStreamParser = function(n, p) {
    return (n <= 0 ?
        _end : 
        bindParser(p,
            bind(_concatParser, timesStreamParser(n - 1, p))));
};

var timesParser = function(n, p) {
    return toArrayParser(timesStreamParser(n, p));
};
    ...
    return function(min, max, p) {
        return (max < min ?
            ...
            bindParser(timesParser(min, p),
                function(start) {
                    return bindParser(maxParser(max - min, p),
                        function(rest) {
...
var sepBy1 = function(sep, p) {
    return bindParser(p,
        bind(_concatParser, manyParser(nextParser(sep, p))));
};
