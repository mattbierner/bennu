 ...
 * @param {Number} max Maximim umber of times to consume p.
 * @param p Parser to consume.
 */
var betweenTimesParser = function(min, max, p) {
    if (max < min) {
        return neverParser();
    } else if (max === min) {
        return timesParser(min, p);
    } else {
        return bindParser(timesParser(min, p),
            bind(_concatToParser, optionalParser(betweenTimesParser(1, max - min, p))));
    }
};
