...
define(["nu/stream", "seshat"], function(stream, seshat) {
    ...
    (ParserState.prototype.first = function() {
        return stream.first(this.input);
    }
    );
    (ParserState.prototype.next = function(x) {
        if (! this._next){
            var s = new ParserState(stream.rest(this.input), this.position.increment(x), this.userState);
        ...
        return function(consume, onErr) {
            ...
            return function TOKEN_PARSER(state, m, cok, cerr, eok, eerr) {
                ...
                if (state.isEmpty()){
                    ...
                    var tok = state.first();
                    return (consume(tok) ? state.next(tok)(state, m, cok, cerr, eok, eerr) : eerr(errorHandler(pos, tok), state, m));
                }
    ...
    var memo = function(p) {
        return function(id) {
            return function(state, m, cok, cerr, eok, eerr) {
                ...
                var entry = Memoer.lookup(m, pos, key);
                if (entry)return cont(entry, [state, m, cok, cerr, eok, eerr]);
                
                return cont(p, [state, m, function(x, pstate, pm) {
                    return cok(x, pstate, Memoer.update(pm, pos, key, function(_, m, cok) {
                        return cok(x, pstate, m);
                    }
                    ));
                }
                , function(x, pstate, pm) {
                    return cerr(x, pstate, Memoer.update(pm, pos, key, function(_, m, cok, cerr) {
                        return cerr(x, pstate, m);
                    }
                    ));
                }
                , function(x, pstate, pm) {
                    return eok(x, pstate, Memoer.update(pm, pos, key, function(_, m, cok, cerr, eok) {
                        return eok(x, pstate, m);
                    }
                    ));
                }
                , function(x, pstate, pm) {
                    return eerr(x, pstate, Memoer.update(pm, pos, key, function(_, m, cok, cerr, eok, eerr) {
