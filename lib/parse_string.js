...
define(["parse/parse", "nu/stream"], function(parse, stream) {
    ...
    var stringParser = function() {
        var impl = function(original, remaining, found) {
            return ((remaining.length === 0) ? parse.always(original) : parse.bind(parse.token(stringEq.bind(undefined, remaining[0]), function(pos, tok) {
                return new parse.ExpectError(pos, original, (found + tok));
            }
            ), function(x) {
                return impl(original, remaining.slice(1), (found + x));
            }
            ));
        }
        ;
        return function(s) {
            return parse.attempt(impl(s, s, ""));
        }
        ;
    }
    ();
