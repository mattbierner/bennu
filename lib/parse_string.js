...
define(["parse/parse", "nu/stream"], function(parse, stream) {
    ...
    var stringParser = function() {
        var impl = function(original, remaining, found, pos) {
            return ((remaining.length === 0) ? parse.always(original) : parse.bind(parse.token(stringEq.bind(undefined, remaining[0]), function(_, tok) {
                return new parse.ExpectError(pos, original, ((tok === null) ? found : (found + tok)));
            }
            ), function(x) {
                return impl(original, remaining.slice(1), (found + x), pos);
            }
            ));
        }
        ;
